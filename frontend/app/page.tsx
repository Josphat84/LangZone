...
export default function Home() {
  ...
  const popularLanguages = ["English", "French", "Spanish", "German", "Chinese", "Japanese"];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 relative overflow-hidden">
      ...
      {/* Floating shapes */}
      <motion.div 
        className="absolute w-64 h-64 bg-teal-200 rounded-full top-10 left-10 opacity-30"
        animate={{ y: [0, 20, 0], x: [0, 10, 0] }}
        transition={{ duration: 10, repeat: Infinity }}
      />
      <motion.div 
        className="absolute w-80 h-80 bg-blue-300 rounded-full bottom-0 right-0 opacity-30"
        animate={{ y: [0, -20, 0], x: [0, -10, 0] }}
      ...
      <section className="relative z-10">
        <div className="max-w-7xl mx-auto px-6 py-20 md:py-28 lg:py-32 flex flex-col-reverse md:flex-row items-center">
          ...
          <div className="md:w-1/2 mb-10 md:mb-0">
            ...
            <img src="/hero-illustration.png" alt="Learning illustration" className="w-full" />
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {features.map((feature, i) => (
            <motion.div 
              ...
              className="flex items-start bg-white p-6 rounded-xl shadow-lg transition-transform"
            >
              <div className="flex-shrink-0 bg-teal-50 p-3 rounded-lg mr-4">{feature.icon}</div>
              <div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {howItWorks.map((step, i) => (
            <motion.div key={i} whileHover={{ scale: 1.05 }} className="bg-gray-50 p-6 rounded-lg text-center shadow-md">
      ...
      {/* Testimonials */}
      <section className="max-w-6xl mx-auto px-6 py-16 relative z-10">
        <h2 className="text-3xl font-bold text-gray-900 text-center mb-12">What Students Say</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <motion.div key={i} whileHover={{ scale: 1.05 }} className="bg-white p-6 rounded-xl shadow-lg text-center">
      ...
      {/* Popular Languages */}
      <section className="max-w-6xl mx-auto px-6 py-16 relative z-10">
        <h2 className="text-2xl font-bold text-gray-900 text-center mb-8">Popular Languages</h2>
        <div className="flex flex-wrap justify-center gap-4">
          {popularLanguages.map((lang, i) => (
            <motion.div key={i} whileHover={{ scale: 1.1 }} className="bg-teal-50 text-teal-600 px-5 py-3 rounded-full shadow-md cursor-pointer">
              {lang}
            </motion.div>
          ))}
        </div>
      ...
      {/* CTA Section */}
      <section className="bg-gradient-to-r from-teal-600 to-blue-600 py-16 relative z-10">
